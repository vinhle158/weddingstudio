...
import { LocalDatabase, User, Customer, Order, OrderStatusHistory, DressInventory, DressRental, Task, TaskUpdate, Role, Objective, ObjectiveKeyResult, ObjectiveProgressUpdate, Notification, ChatMessage } from './src/db_service';
...
async function startServer() {
  ...
  app.post('/api/orders', authenticate, requirePermission('orders.create'), (req: AuthenticatedRequest, res: Response) => {
    ...
    if (!customer_id || !shoot_date || !package_name || package_price === undefined || total_amount === undefined) {
  ...
  app.get('/api/orders/:id', authenticate, requirePermission('orders.view'), (req: AuthenticatedRequest, res: Response) => {
    ...
    const rentals = db.dress_rentals.filter(r => r.order_id === order.id).map(r => {
      const dress = db.dress_inventory.find(d => d.id === r.dress_id);
      return {
        ...r,
        dress_code: dress ? dress.code : 'N/A',
        dress_name: dress ? dress.name : 'N/A',
        dress_category: dress ? dress.category : 'N/A'
      };
    });
  ...
  app.put('/api/orders/:id', authenticate, requirePermission('orders.edit'), (req: AuthenticatedRequest, res: Response) => {
    ...
    if (package_price !== undefined) db.orders[idx].package_price = Number(package_price);
    if (deposit_amount !== undefined) db.orders[idx].deposit_amount = Number(deposit_amount);
    if (total_amount !== undefined) db.orders[idx].total_amount = Number(total_amount);
  ...
  app.get('/api/orders/:id/rentals', authenticate, requirePermission('orders.view'), (req: AuthenticatedRequest, res: Response) => {
    const db = LocalDatabase.get();
    const rentals = db.dress_rentals.filter(r => r.order_id === req.params.id).map(r => {
      const dress = db.dress_inventory.find(d => d.id === r.dress_id);
      return {
        ...r,
        dress_code: dress ? dress.code : 'N/A',
  ...
  // ----------------- DRESS INVENTORY ENDPOINTS -----------------
  app.get('/api/dresses', authenticate, requirePermission('inventory.view'), (req: AuthenticatedRequest, res: Response) => {
    const db = LocalDatabase.get();
    const { status, category } = req.query;

    let result = db.dress_inventory;
    if (status) {
      result = result.filter(d => d.status === status);
    }
    if (category) {
      result = result.filter(d => d.category.toLowerCase() === (category as string).toLowerCase());
    }

    res.json(result);
  });

  app.post('/api/dresses', authenticate, requirePermission('inventory.edit'), (req: AuthenticatedRequest, res: Response) => {
    const { code, name, category, size, color, notes, image_url } = req.body;
    if (!code || !name || !category || !size || !color) {
      return res.status(400).json({ error: 'Thiếu thông tin bắt buộc của trang phục' });
    }

    const db = LocalDatabase.get();
    if (db.dress_inventory.some(d => d.code.toLowerCase() === code.toLowerCase())) {
      return res.status(400).json({ error: 'Mã trang phục này đã tồn tại trong kho' });
    }

    const newDress: DressInventory = {
      id: 'dress-' + LocalDatabase.uuid(),
      code,
      name,
      category,
      size,
    ...
    db.dress_inventory.push(newDress);
    LocalDatabase.save(db);

    res.status(201).json(newDress);
  });

  app.get('/api/dresses/:id', authenticate, requirePermission('inventory.view'), (req: AuthenticatedRequest, res: Response) => {
    const db = LocalDatabase.get();
    const dress = db.dress_inventory.find(d => d.id === req.params.id);
    if (!dress) {
      return res.status(404).json({ error: 'Không tìm thấy sản phẩm trang phục' });
    }
    res.json(dress);
  });

  app.put('/api/dresses/:id', authenticate, requirePermission('inventory.edit'), (req: AuthenticatedRequest, res: Response) => {
    const { id } = req.params;
    const { name, category, size, color, status, notes, image_url } = req.body;

    const db = LocalDatabase.get();
    const idx = db.dress_inventory.findIndex(d => d.id === id);
    if (idx === -1) {
      return res.status(404).json({ error: 'Không tìm thấy trang phục' });
    }

    if (name) db.dress_inventory[idx].name = name;
    if (category) db.dress_inventory[idx].category = category;
    if (size) db.dress_inventory[idx].size = size;
    if (color) db.dress_inventory[idx].color = color;
    if (status) db.dress_inventory[idx].status = status;
    if (image_url !== undefined) db.dress_inventory[idx].image_url = image_url;
    if (notes !== undefined) db.dress_inventory[idx].notes = notes;

    LocalDatabase.save(db);
    res.json(db.dress_inventory[idx]);
  });

  app.get('/api/dresses/:id/rentals', authenticate, requirePermission('inventory.view'), (req: AuthenticatedRequest, res: Response) => {
    const db = LocalDatabase.get();
    const rentals = db.dress_rentals.filter(r => r.dress_id === req.params.id).map(r => {
  ...
  // ----------------- DRESS RENTALS ENDPOINTS -----------------
  app.get('/api/rentals', authenticate, requirePermission('inventory.view'), (req: AuthenticatedRequest, res: Response) => {
    const db = LocalDatabase.get();
    const { status, overdue } = req.query;

    let result = db.dress_rentals.map(r => {
      const dress = db.dress_inventory.find(d => d.id === r.dress_id);
      const order = db.orders.find(o => o.id === r.order_id);
      const customer = order ? db.customers.find(c => c.id === order.customer_id) : null;
      return {
    ...
    if (overdue === 'true') {
      ...
      result = result.filter(r => r.status === 'rented' && r.return_date < today);
    }

    res.json(result);
  });

  app.post('/api/rentals', authenticate, requirePermission('inventory.edit'), (req: AuthenticatedRequest, res: Response) => {
    const { dress_id, order_id, rented_date, return_date, rental_fee, notes } = req.body;
    if (!dress_id || !order_id || !rented_date || !return_date || rental_fee === undefined) {
      return res.status(400).json({ error: 'Thiếu thông tin cho thuê trang phục' });
    }

    const db = LocalDatabase.get();
    const dressIdx = db.dress_inventory.findIndex(d => d.id === dress_id);
    if (dressIdx === -1) {
      return res.status(404).json({ error: 'Không tìm thấy trang phục trong kho' });
    }

    // Rule: Rented items cannot be rented again
    if (db.dress_inventory[dressIdx].status === 'rented') {
      return res.status(400).json({ error: 'Trang phục này hiện đang được thuê, không thể cho thuê tiếp' });
    }

    // Create rental
    const newRental: DressRental = {
      id: 'rental-' + LocalDatabase.uuid(),
      dress_id,
      order_id,
      rented_date,
      return_date,
    ...
    db.dress_rentals.push(newRental);

    // Update dress status to rented
    db.dress_inventory[dressIdx].status = 'rented';

    LocalDatabase.save(db);
    res.status(201).json(newRental);
  });

  app.put('/api/rentals/:id', authenticate, requirePermission('inventory.edit'), (req: AuthenticatedRequest, res: Response) => {
    const { id } = req.params;
    const { return_date, rental_fee, notes } = req.body;

    const db = LocalDatabase.get();
    const idx = db.dress_rentals.findIndex(r => r.id === id);
    if (idx === -1) {
      return res.status(404).json({ error: 'Không tìm thấy lịch sử thuê đồ' });
    }

    if (return_date) db.dress_rentals[idx].return_date = return_date;
    if (rental_fee !== undefined) db.dress_rentals[idx].rental_fee = Number(rental_fee);
    if (notes !== undefined) db.dress_rentals[idx].notes = notes;

    LocalDatabase.save(db);
    res.json(db.dress_rentals[idx]);
  });

  app.post('/api/rentals/:id/return', authenticate, requirePermission('inventory.edit'), (req: AuthenticatedRequest, res: Response) => {
    const { id } = req.params;
    const db = LocalDatabase.get();
    const rentalIdx = db.dress_rentals.findIndex(r => r.id === id);
    if (rentalIdx === -1) {
      return res.status(404).json({ error: 'Không tìm thấy lịch sử thuê đồ' });
    }

    const rental = db.dress_rentals[rentalIdx];
    if (rental.status === 'returned') {
      return res.status(400).json({ error: 'Sản phẩm này đã được trả từ trước' });
    }
    ...
    // Set dress status to available
    const dressIdx = db.dress_inventory.findIndex(d => d.id === rental.dress_id);
    if (dressIdx !== -1) {
      db.dress_inventory[dressIdx].status = 'available';
    }

    LocalDatabase.save(db);
    res.json({ success: true, rental, dress: db.dress_inventory[dressIdx] });
  });
  ...
  app.get('/api/dashboard/summary', authenticate, (req: AuthenticatedRequest, res: Response) => {
    ...
    // Dress status counts
    const dressStatuses = { available: 0, rented: 0, maintenance: 0 };
    db.dress_inventory.forEach(d => {
      if (dressStatuses[d.status] !== undefined) {
        dressStatuses[d.status]++;
      }
    });
    ...
    // Count of overdue dress rentals
    const overdueRentalsCount = db.dress_rentals.filter(r => 
      r.status === 'rented' && 
      r.return_date < todayStr
    ).length;
    ...
    res.json({
      ...
      dresses: {
        total: db.dress_inventory.length,
        by_status: dressStatuses
      },
      tasks: {
        total: totalTasks,
        done: doneTasks,
  ...
  app.get('/api/dashboard/due-rentals', authenticate, (req: AuthenticatedRequest, res: Response) => {
    const db = LocalDatabase.get();
    const todayStr = new Date().toISOString().split('T')[0];

    const dueRentals = db.dress_rentals
      ...
      .map(r => {
        ...
        return {
          ...
          is_overdue: r.return_date < todayStr,
          order_code: order ? order.order_code : 'N/A'
        };
      })
